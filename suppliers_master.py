...
def _normalize_match_score(value: Any) -> str:
    """
    Keep match_score consistently string-like.
    This avoids dtype issues with pandas StringDtype columns when assigning floats.
    """
    if value is None:
        return ""
...
def build_suppliers_master(
    ...
    for idx, row in master.iterrows():
        ...
        if len(unique_matched_ids) == 1:
            ...
            score = _normalize_match_score(first_match.get("match_score", ""))
            ...
            method, score, conf = "exact_key", _normalize_match_score(100.0), "high"
        else:
            ...
            if containment_unique is not None and best_score >= (high - 2) and not weak_query:
                ...
                method, score, conf = "token_containment", _normalize_match_score(float(best_score)), "high"
            elif best_rec is not None and best_score >= high and not weak_query:
                matched_id = best_rec["supplier_id"]
                method, score, conf = "fuzzy", _normalize_match_score(float(best_score)), "high"
